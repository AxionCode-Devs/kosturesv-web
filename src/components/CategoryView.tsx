...
export default function CategoryView() {
  ...
  useEffect(() => {
    ...
    if (hash) {
      // Retraso ligero para asegurar que el DOM ha renderizado las tarjetas
      setTimeout(() => {
  ...
  }, [categoriaSlug, location.hash]);

  // Buscar el título de la categoría basado en el slug
  const categoryTitles: Record<string, string> = t.catalogo.categoryTitles;
            ...
            {productos.map((item, index) => (
              ...
              >
                <div
                  ...
                  className={`relative h-[350px] md:h-[450px] w-full group overflow-hidden cursor-pointer bg-black rounded-none transition-all duration-1000 ${
                    highlightedId === item.codigo 
                      ? "animate-highlight-pulse z-20" 
                      : ""
                  }`}
                >
                  <img
                    src={item.imagen}
                    alt={item.nombre}
                    className="w-full h-full object-cover opacity-90 transition-transform duration-[2s] group-hover:scale-105 group-hover:opacity-100 rounded-none"
                    onError={(e) => {
                      e.currentTarget.style.display = "none";
                    }}
                  />
                  
                  {/* Overlay Oscuro solo en la parte inferior */}
                  <div className="absolute bottom-0 left-0 right-0 h-1/2 bg-gradient-to-t from-[#1C2E3A]/90 to-transparent transition-all duration-700 group-hover:from-[#1C2E3A]/80" />
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-8 z-10 text-left">
                    <h3 
                      className="text-white text-xl md:text-2xl font-bold uppercase tracking-wider mb-2 drop-shadow-md" 
                      style={{ fontFamily: "var(--font-heading)" }}
                    >
                      {item.nombre}
                    </h3>
                ...
                <div className="space-y-5 mb-10">
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-0.5">{productoSeleccionado.origen || t.modal.originValue}</p>
                    </div>
                  </div>
                </div>
              </div>
